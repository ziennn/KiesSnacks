...
var food =JSON.parse(localStorage.getItem('food'));
    ...
    var prinfManage =`<tr>
      <td><div>
        <div class="cart_img_box float-left bg-warning" style="width: 15%;height: 80px;">
  ...
  <div class="btn btn-warning  text-white mr-2" onclick="UpdateFood(`+food[i].id+`)" style="width: 16%;border-radius: 10px; background: #86c232; border: #86c232"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></div> </td>
</tr>`;
...
function paymentPrinf(){
    ...
    var prinf_payment_cart = `<tr style="border-bottom:1px solid #ccc;">
      ...
      <td class="text-center"><button id="status_prinf`+paymentFood[i].id_payment+`" onclick="verify_payment(`+paymentFood[i].id_payment+`)" class="btn btn-warning text-white" style="background: #86c232; border: #86c232">`+paymentFood[i].status+`</button></td>
    </tr>
    ` 
...
function paymentPrinfDash(){
    ...
    var prinf_payment_cart = `<tr style="border-bottom:1px solid #aaa;">
...
function verify_payment(id){
    for(let i = 0; i < paymentFood.length; i++) {
      if (id == paymentFood[i].id_payment) {
        paymentFood[i].status="On Delivery";
        localStorage.setItem("paymentFood", JSON.stringify(paymentFood));
...
function commentPrinf(){
  ...
  var account =JSON.parse(localStorage.getItem('account'));
    for(let i = 1; i < commentCustomer.length; i++) {
                            /*<td>`+commentCustomer[i].idPayment+`</td>*/
    var prinf_comment = `
    <td>`+account[commentCustomer[i].idUser].username+`</td>
    <td>`+commentCustomer[i].content_comment+`</td>
    ` ;
    document.getElementById("commentPrinfPayment"+commentCustomer[i].idPayment).innerHTML ="Feedback: "+commentCustomer[i].content_comment;
    ...
    if (username != '' && password !='' && repassword !='' && password == repassword) {
      for(let i = 0; i < account.length; i++) {
        if (account[i].username== username) {
          alert("User already exist! Create new user.");
...
var account =JSON.parse(localStorage.getItem('account'));
    ...
    var prinfManage =`<div class="col-4" style="height: 100px;padding:10px; padding-right:50px;">
      ...
      <div class="btn btn-warning mr-1 text-white" onclick="editUserAdmin(`+account[i].id+`)" style="width: 40%;border-radius: 10px;  background: #86c232; border: #86c232"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></div>
...
function onloadAll1(){
      ...
      commentPrinf();
      ManagementUser();
      localStorage.clear(commentCustomer);
      localStorage.clear(account_info);
      localStorage.clear(id_payment);
    }
/*--------------------------------------------------------------------------------------------*/